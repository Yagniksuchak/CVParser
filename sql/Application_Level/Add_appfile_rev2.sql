...
#calculate required number for each type of file
CREATE TABLE mywork.appfile3
SELECT campus_id, id, job_number, sum(is_required) as required, filename FROM
mywork.appfile2  
GROUP BY campus_id, id, job_number, filename
ORDER BY campus_id, id, filename;
...
#merge total count with required count
