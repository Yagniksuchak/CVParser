...
function(q, doi = NULL, year = NULL, search_num, search = 1) 
{
    ...
    if(!is.null(df)){
            ...
            if(search==3){
                    keep_cols = c("doi", "score", "normalizedScore", "title", "fullCitation", "coins", "year")
                    search_result = as.list( rep( NA, length(keep_cols) ) )
                    names( search_result ) = keep_cols
                    df = data.frame(search_result)
                    return(df)
            }
    }
...
function( search_result, original_cite, cv_name ) 
{
    ...
    if( nrow(search_result) == 0 ) {
        ...
        search_result$citation_rank = rownames(search_result)
    ...
    # Now we are keeping only top 2 records.
    ordering = order( filtered_results$merge_score, decreasing = TRUE )
    filtered_results = head( filtered_results[ ordering, ], 2 )
...
function(cite_list, doi_list, year_list, search_num = 5, id = "sample_id", cv_name)
{
