...
#source("~/Documents/ucrecruit/uc_recruit_cv_parser/CVRead/R/procMiner.R")
source(file.path(dirname(dirname(getwd())),"CVRead/R/procMiner.R"))
...
#options(PDF2TXT = "~/anaconda/lib/python2.7/site-packages/pdfminer/tools/pdf2txt.py")
#options(PDF2TXT = "/Users/gsr/Documents/pdfminer/tools/pdf2txt.py")
#options(PDF2TXT = file.path(dirname(dirname(getwd())),"pdfminer/tools/pdf2txt.py"))
...
function( input_full_path, output_full_path, error_file )
{
    mapply( function(pdf_doc, filename, error_file) {
       # browser()
        ...
        doc = try( pdfMinerDoc(pdf_doc,removeHeader = FALSE), silent = TRUE)
...
pdf_to_xml = 
    ...
    #   out of memory issue
function( input_dir, output_dir = "../CV_XML", via_cmd = TRUE ) 
{
    ...
    mapply(dir.create,output_dir,showWarnings=FALSE)
...
#            output_filename = "~/Documents/cv/temp_files/xml_output_path.txt"
            input_filename= file.path(dirname(dirname(dirname(dirname(getwd())))),"cv/temp_files/pdf_input_path.txt")
            output_filename = file.path(dirname(dirname(dirname(dirname(getwd())))),"cv/temp_files/xml_output_path.txt")
