...
get_bulleted_lines = 
        ...
        function(text)
        {
                ...
                x = sort(table(value),decreasing=TRUE)[1]
                (names(x) == value)*1
...
function( citation_text, groups ) 
{
    ...
    mask = cite_length > 85 & cite_length < 4000
...
function( filename, df, name_index, pass = 1 )
{
    ...
    if( pass == 1 ) {
        ...
        get_citations_helper( df, "bulleted" )            
    } else if( pass == 5 ) {
...
#we'll save the file level data for doing fileLevelTesting.
# file level elements: FileName, StartSection, EndSection, NumOfPublications Found.

function( cv_filename, cv_name, pub_filename, section_filename, print_text = FALSE,log_file=FALSE,fileLevelTest=TRUE)
{
    # for testing
    if( FALSE ) {
        cv_name = "rubin"
    ...
    if(fileLevelTest & !file.exists("fileLevelTestOutput.csv")){
                write("FileName,StartSection,EndSection,NumOfPublication","fileLevelTestOutput.csv")
 }
    extension = tolower( substring( cv_filename, nchar(cv_filename) - 3) )
    baseFilename= basename(cv_filename)
    baseFilename=substring(baseFilename,1,nchar(baseFilename)-4)
    if(log_file & file.exists(sprintf("%s.txt",baseFilename))){
            return(NULL)
    }
    write(baseFilename,sprintf("%s.txt",baseFilename))
    
    output = parse_cv( cv_filename,section_filename, short_text = FALSE )
    df = output[[1]]
    found_sections = output[[2]]
    ...
    if(is.null(name_index$start_index) || is.null(name_index$end_index)){
            return(NULL)
    ...
    #add bulleted information
    df_text$bullet = get_bulleted_lines(df_text$text)
    ...
    if(log_file){
            baseFilename= basename(cv_filename)
            baseFilename=substring(baseFilename,1,nchar(baseFilename)-4)
            write(as.character(name_index),sprintf("%s.txt",baseFilename),append = TRUE)
            write("Citations:",sprintf("%s.txt",baseFilename),append = TRUE)
            write(citations,sprintf("%s.txt",baseFilename),append = TRUE)
    }
    
    if(fileLevelTest){
            arr = list(baseFilename,name_index[[2]],name_index[[4]],length(citations))
            arr=paste(arr, collapse = ',')
            write(arr,"fileLevelTestOutput.csv",append = TRUE)
            #REMOVE THIS RETURN STATEMENT TO GET CROSSREF RESULT
            return(NULL)
    ...
    crossrefResult=query_crossref( citations, doi, year, id = citation_id, cv_name = cv_name ) 
    crossrefResult$startSection =name_index[[2]]
    crossrefResult$endSection =name_index[[4]]
    crossrefResult
}
