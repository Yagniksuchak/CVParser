...
function(new_sections, filename = "~/Document/ucrecruit/uc_recruit_cv_parser/parse_citations/text_files/section_names.txt")
{
...
function(vect, section_names, use_sum = TRUE)
{
    ...
    # build regular expression from character vector of section names
        ...
        section_names = fix_group_text(tolower(section_names))
        section_reg = create_reg(section_names)
        vect = tolower(vect)
        index = grep(section_reg, vect)
        return(length(index))
    
#     vect = tolower( as.character(vect) )
#     section_names = tolower(section_names)
...
function(df, section_names, group_list, baseFileName,min_sections = get_min_sections(df), max_sections = 50, 
         pass = 1, to_print = FALSE,to_file = TRUE) 
{
    ...
    mask = sapply(group_by, function(vect) ( (length(vect) < max_sections) ))
    ...
    if( to_print || to_file & length(group_by)!=0) {
        ...
        group_frame=mapply( function(group, name) {
        ...
        mapply(function(x,heading,baseFileName){
                currFile = sprintf("%s.txt",baseFileName)
                x=as.character(x)
                write(sprintf( "group %s",heading),currFile,append = TRUE)
                write(x,currFile,append=TRUE)
                write("\n",currFile,append=TRUE)
                
                                                },group_frame,heading=1:length(group_frame),baseFileName)
    ...
    if( pass == 1 ) {
        ...
        group_list = list(df$caps_tf, df$text_size, df$font_bold, df$font_italic, df$font)
        cv_search_tree(df, section_names, group_list,baseFileName , min_sections, max_sections, pass = 2)
    } else if ( pass == 2 ){
        # next pass here
    }  
}
...
function(filename, section_filename = "/Users/gsr/Documents/ucrecruit/uc_recruit_cv_parser/parse_citations/text_files/section_names.txt", 
...
#     }
    ...
    doc = 
        if( extension == ".pdf" ) {
            pdfMinerDoc(filename,removeHeader = FALSE)
    ...
    new_sections = cv_search_tree(df, section_names, group_list,baseFilename, to_print = FALSE)
